...
func TestDefectiveGlyphsOmittedFromPairwise(t *testing.T) {
	svc := newTestService(t)
	b, err := svc.CreateBatch("BX-DEF", "残缺字形探针")
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := svc.MarkGlyphDefective(g3.ID); err != nil {
		t.Fatal(err)
	}
	...
	for _, r := range rels {
		if r.SourceGlyph == g3.ID || r.TargetGlyph == g3.ID {
			t.Fatalf("defective glyph %d must not appear in relations", g3.ID)
		}
	}
	_ = g1
	_ = g2
